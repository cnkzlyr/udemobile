...
// UDF YÜKLEME (Tauri Invoke kullanılıyor)
document.getElementById('fileInput')?.addEventListener('change', async (e) => {
  ...
  try {
    ...
    // DEĞİŞİM: Rust fonksiyonunu yerel olarak çağırıyoruz
    const xmlContent = await invoke<string>('extract_udf', { data: bytes });

    const parser = new DOMParser();
    globalXmlDoc = parser.parseFromString(xmlContent, "application/xml");
    ...
    bodyZone.innerText = "Sistem Hatası: " + err;
  }
});
...
// UDF KAYDETME (Tauri Invoke kullanılıyor)
document.getElementById('menuSaveUdf')?.addEventListener('click', async () => {
  ...
  try {
    ...
    // DEĞİŞİM: Rust'a XML'i gönderip ZIP baytlarını alıyoruz
    const zipBytes = await invoke<number[]>('package_udf', { xmlContent: finalXml });
...
document.getElementById('menuSavePdf')?.addEventListener('click', () => window.print());
